//
// Created by Zhouyou Gu on 17/10/24.
//
...
main (int argc, char *argv[])
{
  ...
  uint32_t interval_in_us = 10000; // Interval between packets in microseconds (not in used)
  ...
  int simTime = 2; // Simulation time in seconds not in use
  ...
  // Set up mobility for APs STAs (fixed positions)
  ...
  // Create propagation loss model (matrix loss model)
  ...
  double mini_application_time_pad_second = 1e-5;
  ...
  for (int i = 0; i < n_sta; ++i)
    {
      ...
      s->SetAttribute ("PacketSize", UintegerValue (packetSize-UDP_IP_WIFI_HEADER_SIZE));
  ...
  myGymEnv->n_packet = numPackets;
  myGymEnv->m_serverApps.Add(ServerApps);
  myGymEnv->m_clientApps.Add(ClientApps);
  ...
  for (uint32_t j = 0; j < n_sta; j++)
    {
      ...
      auto payloadsDuration_us =
          v->GetPayloadDuration ((packetSize), txVector, v->GetPhyBand ())
              .GetMicroSeconds ();
  ...
  Simulator::Stop (myGymEnv->simulation_time + Seconds(0.1));
