...
MyGymEnv::GetObservationSpace()
{
  ...
  Ptr<OpenGymBoxSpace> pkc = CreateObject<OpenGymBoxSpace> (0., 1000000000., shape_aoi, dtype);
...
MyGymEnv::GetObservation()
{
  ...
  if (is_simulation_end){
    for (uint32_t j = 0; j < m_n_sta; j++){
      ...
      pm_3->AddValue (del);
      float pkc = (float) DynamicCast<UdpServer>(m_serverApps.Get(j))->GetReceived();
      pm_4->AddValue (pkc);
    }
  } else {
    for (uint32_t j = 0; j < m_n_sta; j++){
...
//  NS_LOG_INFO ("MyGetObservation: " << data);
//  NS_LOG_UNCOND ("---" << aoi);
...
MyGymEnv::ExecuteActions(Ptr<OpenGymDataContainer> action)
{
  ...
  NS_LOG_FUNCTION (this<< "loading action done");
  ...
  counter = 0;
  NS_LOG_FUNCTION (this<< "filling path loss done");
  ...
  NS_LOG_FUNCTION (this<< "loading twtcfg done");

  Time max_sim_duration = MicroSeconds(0);
  NS_LOG_FUNCTION (this<< "app size, c-"<< m_clientApps.GetN() << ", a-"<<m_serverApps.GetN());
  double mini_application_time_pad_us = 1;
    for (uint32_t i = 0; i < m_n_sta; i++)
    {  
      ...
      NS_LOG_FUNCTION (this<< "setting twtcfg for STA-"<< i
      ...
      v->GetTxop ()->GetWifiMacQueue ()->SetMaxDelay (MicroSeconds(twtduration->GetValue(i)+mini_application_time_pad_us));

      DynamicCast<UdpClient>(m_clientApps.Get(i))->SetAttribute ("Interval", TimeValue(MicroSeconds(twtperiodicity->GetValue(i))));
      NS_LOG_FUNCTION (this<< "setup packet interval, c-id-" << i);

      auto app_start_time = MicroSeconds(twtstarttime->GetValue(i))+MicroSeconds(twtoffset->GetValue(i))-MicroSeconds(mini_application_time_pad_us);
      auto app_stop_time = MicroSeconds(twtstarttime->GetValue(i))+MicroSeconds(n_packet*twtperiodicity->GetValue(i))+MicroSeconds(mini_application_time_pad_us);
      NS_LOG_FUNCTION (this<< "compute app start and end time, " << app_start_time.GetSeconds() << "-" << app_stop_time.GetSeconds());
      m_clientApps.Get(i)->SetStartTime(app_start_time);
      m_clientApps.Get(i)->SetStopTime (app_stop_time);
      m_serverApps.Get(i)->SetStartTime(app_start_time);
      m_serverApps.Get(i)->SetStopTime (app_stop_time);
      NS_LOG_FUNCTION (this<< "setup app starting time, sta-id-" << i);

      NS_LOG_FUNCTION (this<< "previos sim duration (s) as " << max_sim_duration.GetSeconds());
      if(app_stop_time.Compare(max_sim_duration)>0){
        max_sim_duration = app_stop_time;
      }
      NS_LOG_FUNCTION (this<< "updated sim duration (s) as " << max_sim_duration.GetSeconds());
    }
  NS_LOG_FUNCTION (this<< "filling twtcfg and apptime done");

  simulation_time = max_sim_duration;
  return true;
}
