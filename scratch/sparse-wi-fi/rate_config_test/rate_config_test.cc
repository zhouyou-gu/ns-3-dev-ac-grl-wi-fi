...
main (int argc, char *argv[])
{
  ...
  int n_ap = 4; // Number of Access Points (APs)
  int n_sta = 40; // Number of Stations (STAs)

  uint32_t simSeed = 1000; // Simulation seed
  int simTime = 10; // Simulation time in seconds
  ...
  positionAlloc_ap->Add (Vector (50, 50, 0.0));
  positionAlloc_ap->Add (Vector (-50, 50, 0.0));
  positionAlloc_ap->Add (Vector (50, -50, 0.0));
  positionAlloc_ap->Add (Vector (-50, -50, 0.0));
  ...
  // Set up mobility for STAs (random positions)
  MobilityHelper mobility_sta;
  Ptr<RandomRectanglePositionAllocator> positionAlloc_sta =
      CreateObject<RandomRectanglePositionAllocator> ();
  Ptr<UniformRandomVariable> rnd = CreateObject<UniformRandomVariable> ();
  rnd->SetAttribute ("Min", DoubleValue (-100));
  rnd->SetAttribute ("Max", DoubleValue (100));
  ...
  wifiPhy.Set ("CcaEdThreshold", DoubleValue (-75.0));
  ...
  for (uint32_t j = 0; j < n_sta; j++)
    {
      ...
      for (const auto &bps : bps_array)
      {
        ...
        double ps =
            ppv->GetChunkSuccessRate (mode, txVector,
