...
MyGymEnv::GetObservation()
{
  ...
  std::vector<uint32_t> shape_path_loss_ap_ap = {(uint32_t)m_n_ap*(uint32_t)m_n_ap,};
  Ptr<OpenGymBoxContainer<double> > path_loss_ap_ap = CreateObject<OpenGymBoxContainer<double> >(shape_path_loss_ap_ap);
  for (uint32_t i = 0; i < m_n_ap; i++){
    for (uint32_t j = 0; j < m_n_ap; j++){
      ...
      double loss = m_loss_model->CalcRxPower(0,A,B);
  ...
  std::vector<uint32_t> shape_path_loss_sta_ap = {(uint32_t)m_n_ap*(uint32_t)m_n_sta,};
  Ptr<OpenGymBoxContainer<double> > path_loss_sta_ap = CreateObject<OpenGymBoxContainer<double> >(shape_path_loss_sta_ap);
  for (uint32_t i = 0; i < m_n_ap; i++){
  ...
  Ptr<OpenGymBoxContainer<double> > loss_ap_ap = DynamicCast<OpenGymBoxContainer<double> >(data->Get("loss_ap_ap"));
  Ptr<OpenGymBoxContainer<double> > loss_sta_ap = DynamicCast<OpenGymBoxContainer<double> >(data->Get("loss_sta_ap"));
